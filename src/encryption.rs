...
impl RscpEncryption {
    ...
    pub fn encrypt(&mut self, data: Vec<u8>) -> Result<Vec<u8>> {
        ...
        let result =
            RijndaelCbc::<ZeroPadding>::new(&self.key, BLOCK_SIZE)?.encrypt(&self.enc_iv, data)?;
...
fn test_encryption() {
    ...
    let mut res = enc
        .encrypt("00011122233344455566677788899900".as_bytes().to_vec())
        .unwrap();
    assert_eq!(
        res,
        vec![
            0x8d, 0xfa, 0xc7, 0x4d, 0xcb, 0x33, 0x0b, 0x0d, 0x23, 0xe3, 0x4e, 0xfd, 0xe4, 0x28,
            0xcb, 0xcd, 0x9b, 0x3d, 0x8c, 0xe9, 0x2a, 0xc5, 0x3a, 0x26, 0xf1, 0x17, 0x41, 0x87,
            0xa7, 0x1a, 0x48, 0xca
        ],
        "Test 32 byte encryption"
    );

    res = enc
        .encrypt("000111222333444555666777888999000".as_bytes().to_vec())
        .unwrap();
    assert_eq!(
        res,
        vec![
            0xc0, 0x50, 0x27, 0xbb, 0xd6, 0x0d, 0xf4, 0xa3, 0xc1, 0x98, 0xd9, 0xee, 0x2d, 0xa9,
            0xf3, 0xf6, 0x34, 0x04, 0x76, 0x5b, 0xce, 0x0b, 0x12, 0xa9, 0x9d, 0x43, 0x87, 0x8b,
            0x78, 0xe8, 0xee, 0x33, 0x6c, 0xbc, 0x00, 0x44, 0xcf, 0xe2, 0x86, 0x94, 0xf1, 0xde,
            0x9e, 0x47, 0x24, 0xe5, 0xab, 0x59, 0x8f, 0x64, 0x0f, 0xf4, 0x19, 0x62, 0x82, 0x84,
            0x34, 0xe2, 0x00, 0x9a, 0xcc, 0x13, 0x89, 0xfd
        ],
        "Test 33 byte encryption"
    );
}
...
fn test_decryption() {
    ...
    assert_eq!(
        res,
        "00011122233344455566677788899900".as_bytes(),
        "Test 32 byte decryption"
    );
